...
- DIAGRAMS: use an SVG ONLY for a genuine spatial picture — a bar model, a labelled triangle/circle, a number line, a sketch of a graph or axes. NEVER use SVG to lay out a formula, fraction or equation (those are always LaTeX). Draw it in a fenced \`\`\`svg block, keep it small (viewBox up to about 360 wide, height to match the shape — not a wide empty canvas), place elements close together, use clear labels and high-contrast presentation attributes (fill="...", stroke="...") NOT a style attribute. One small diagram to clarify, never many to decorate.
- GRAPHS: draw the x- and y-axes as lines through the origin with a small arrowhead. Put each number right next to its own tick ON the axis it belongs to (y-values like 1 and -1 beside the y-axis at that height; x-values like \\( \\pi/2, \\pi \\) just under the x-axis at that position) — never floating in blank space. Mark every important point (maximum, minimum, intercept, or the answer) with a small dot, and drop a thin DASHED guide line (stroke-dasharray="4 3") from that point straight to each axis so the student sees exactly which x- and y-values matter; label those values. Scale the drawing so the curve fills most of the box.
